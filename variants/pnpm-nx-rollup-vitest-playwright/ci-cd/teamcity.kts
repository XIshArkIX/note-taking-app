...
version = "2024.03"
val VARIANT = "pnpm-nx-rollup-vitest-playwright"
project {
    vcsRoot(Stash)
...
object Typecheck : BuildType({ name="Typecheck"; vcs { root(Stash) }
    ...
    steps { script { name="turbo typecheck"; scriptContent="""
        corepack enable && corepack prepare pnpm@latest --activate
        T0=$(date +%s%3N)
        pnpm run typecheck
        DUR=$(( $(date +%s%3N) - T0 ))
...
object Build : BuildType({ name="Build + Bundle Stats"; vcs { root(Stash) }
    params { param("env.TURBO_TOKEN", "%env.TURBO_TOKEN%"); param("env.TURBO_TEAM", "%env.TURBO_TEAM%") }
    dependencies {
    ...
    steps { script { name="turbo build + stats"; scriptContent="""
        corepack enable && corepack prepare pnpm@latest --activate
        T0=$(date +%s%3N)
        pnpm run build  # nx orchestrates rollup + next
        DUR=$(( $(date +%s%3N) - T0 ))
        echo "##teamcity[buildStatisticValue key='buildMs' value='${DUR}']"
        VARIANT="$VARIANT" BUILD_DIR=apps/notes-app/.next BUILD_TIME=${DUR} node scripts/bundle-stats.mjs
    ...
    """.trimIndent() } }
    artifactRules = "pipeline-report.json\npackages/notes-schema/dist/ => schema-dist.zip" })
